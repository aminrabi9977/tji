...
async def handle_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Immediate response before authentication starts
    auth_message = await update.message.reply_text("🔄 در حال اعتبارسنجی هویت...")
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle news link and process it."""
    # Get the URL that user sent
    url = update.message.text
    
    # Store the URL in user_data
    context.user_data['original_url'] = url
    
    # Send immediate response to show processing is starting
    processing_message = await update.message.reply_text("📝 در حال پردازش درخواست...")
    
    try:
        # First analysis message - immediately after URL entry
        analysis_message = await update.message.reply_text("🔍 استخراج خبر از لینک ارسالی ...")
        ...
        # Update before upload begins
        upload_message = await update.message.reply_text("در حال بارگذاری اطلاعات در سایت تجارت نیوز...")
        
        # Upload to WordPress with the original URL for the source field
        await upload_playwright(
            url="https://tejaratnews.com/wp-login.php",
            select_services=context.user_data['service'],
            username=context.user_data['username'],
            password=context.user_data['password'],
            original_url=context.user_data['original_url']  # Pass the original URL for the source field
        )
